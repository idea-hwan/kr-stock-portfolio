...
def _snapshot_by_company(conn: sqlite3.Connection, as_of: str) -> dict[str, tuple[str, tuple]]:
    """company -> (computed_at, (op_ps, ni_parent_ps, cfo_ps, fcf_ps)) for ttm_end_term=as_of, latest computed_at."""
    rows = conn.execute(
        """
        SELECT company, computed_at, op_ps, ni_parent_ps, cfo_ps, fcf_ps
        FROM ttm_valuation_series
        WHERE anchor_term = ? AND ttm_end_term = ?
        """,
        (as_of, as_of),
    ).fetchall()
    best: dict[str, tuple[str, tuple]] = {}
    for r in rows:
        co, ct = str(r[0]), str(r[1])
        tup = (r[2], r[3], r[4], r[5])
        if co not in best or ct > best[co][0]:
            best[co] = (ct, tup)
    return best


def _val_n_terms(conn: sqlite3.Connection, company: str, anchor: str, computed_at: str, as_of: str) -> int:
    row = conn.execute(
        """
        SELECT COUNT(DISTINCT ttm_end_term) FROM ttm_valuation_series
...
def _last_n_quarters_all_ni_parent_positive(
    ...
    """
    ttm_end_term ≤ as_of 분기만 모음 → 정렬 후 **마지막 min_q개 분기**마다 ni_parent_ps > 0.
    전체 이력이 min_q 미만이면 실패. 동일 분기 중복 행은 마지막 행 기준.
    """
    rows = conn.execute(
        """
        SELECT ttm_end_term, ni_parent_ps
        FROM ttm_valuation_series
        WHERE company = ? AND anchor_term = ? AND computed_at = ? AND ttm_end_term <= ?
...
def large_value_passing_names(
    *,
    ...
    """
    §3.2 스모크 통과 종목명 (mcap_top_200 파일 순서).
    반환: (as_of, names).
    """
...
large_asset_passing_names = large_value_passing_names
...
def main() -> int:
    ...
    try:
        as_of, passed = large_value_passing_names(
    ...
    print(f"PASS (≥{args.min_q} terms, last {args.min_q} consecutive: ni_parent_ps > 0 each): {len(passed)}")
    if args.list_pass:
        for n in passed:
            print(n)
    return 0
